...
def myMakeup(BASE_DIR,img_path,timestamp):
    '''
    美颜模块
    @refer: https://github.com/QuantumLiu/AIMakeup
    ...
    返回原图片和生成图片相对前端的路径
    '''
    image_save = BASE_DIR+"\\results\\makeup_"+timestamp+".jpg"

    predictor_path = BASE_DIR+'\\data\\shape_predictor_68_face_landmarks.dat'

    makeupBeauty(img_path,image_save,predictor_path)

    result = {
        'result_list':[
            {
                'title':'妆前',
                'img_url':'\\posts\\post_'+timestamp+'.jpg'
            },{
                'title':'妆后',
                'img_url':'\\results\\makeup_'+timestamp+'.jpg'
            }
        ]
    }
...
class Organ():
    ...
    def brightening(self,rate=0.7,confirm=True):
        '''
        提升鲜艳度
        arguments:
        ...
        if confirm:
            ...
            self.patch_hsv[:,:,1]=np.minimum(self.patch_hsv[:,:,1]+patch_new,200).astype('uint8')
            self.im_bgr[:]=cv2.cvtColor(self.im_hsv, cv2.COLOR_HSV2BGR)[:]
            self.update_temp()
        else:
            self.patch_hsv_temp[:]=cv2.cvtColor(self.patch_bgr_temp, cv2.COLOR_BGR2HSV)[:]
            ...
            patch_new=cv2.GaussianBlur(patch_new,(3,3),0)
            self.patch_hsv_temp[:,:,1]=np.minimum(self.patch_hsv[:,:,1]+patch_new,200).astype('uint8')
            self.patch_bgr_temp[:]=cv2.cvtColor(self.patch_hsv_temp, cv2.COLOR_HSV2BGR)[:]
        
    def smooth(self,rate=0.9,ksize=None,confirm=True):
        '''
        磨皮
        arguments:
...
class Makeup():
    ...
    化妆器
    '''
    def __init__(self,predictor_path="F:/movieData/Beauty/shape_predictor_68_face_landmarks.dat"):
        self.photo_path=[]
...
def makeupBeauty(img_path,image_save,predictor_path):
    '''
    @param
    img_path: 待处理的图片路径
    image_save: 生成图片存储路径
    predictor_path: 数据文件路径
    '''
    path=img_path

    mu=Makeup(predictor_path=predictor_path)
    im,temp_bgr,faces=mu.read_and_mark(path) # im_bgr,temp_bgr,self.get_faces(im_bgr,im_hsv,temp_bgr,temp_hsv,fname)人脸特征(x,y)坐标的矩阵

    for face in faces[path]:  #{name:[Face(im_bgr,im_hsv,temp_bgr,temp_hsv,np.array([[p.x, p.y] for p in self.predictor(im_bgr, rect).parts()]),i) for i,rect in enumerate(rects)]}
        face.whitening()
        face.smooth(0.8)
        face.organs['forehead'].whitening()
        face.organs['forehead'].smooth(0.8)
        face.organs['mouth'].brightening()
        face.organs['mouth'].smooth(0.8)
        ...
        face.organs['nose'].smooth(0.8)
        face.organs['nose'].sharpen()
        face.sharpen()
        
    outcome=image_save
    cv2.imwrite(outcome,im.copy())
    return outcome